...
object CopyCommand extends DefaultJsonProtocol{
  def withColumns(copyOptions:String,tableName:String, columns:Seq[String]) = CopyCommand(copyOptions,columns.mkString(","),tableName)
  implicit val format: JsonFormat[CopyCommand] = jsonFormat3(CopyCommand.apply)
}

case class RedshiftDestinationConfiguration(
  ClusterJDBCURL: Token[String],
...
object RedshiftDestinationConfiguration extends DefaultJsonProtocol {
  implicit val format: JsonFormat[RedshiftDestinationConfiguration] = jsonFormat8(RedshiftDestinationConfiguration.apply)
}
...
object `AWS::KinesisFirehose::DeliveryStream` extends DefaultJsonProtocol{
  ...
  def redshift(name:String,
    RedshiftDestinationConfiguration: RedshiftDestinationConfiguration,
    DeliveryStreamName: Option[String] = None,
    DeliveryStreamType: Option[DeliveryStreamType] = None,
    DependsOn: Option[Seq[String]] = None,
    Condition: Option[ConditionRef] = None)= {
   new `AWS::KinesisFirehose::DeliveryStream`(name,DeliveryStreamName,DeliveryStreamType,
     RedshiftDestinationConfiguration = RedshiftDestinationConfiguration,
     DependsOn = DependsOn,
     Condition = Condition)
  }
